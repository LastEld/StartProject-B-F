...
from sqlalchemy import (
    Column, Integer, String, Boolean, JSON, DateTime, func, Index, ForeignKey
)
from sqlalchemy.orm import relationship # Added for relationship
import sqlalchemy as sa  # Для server_default=sa.false()
from app.models.base import Base
# Ensure User model can be imported, adjust path if necessary.
# from app.models.user import User # Assuming direct import works
...
class Plugin(Base):
    ...
    # author: str = Column(String(128), nullable=True, doc="Автор/Email") # Old field
    author_id: int = Column(Integer, ForeignKey("users.id"), nullable=False, index=True, doc="ID автора плагина (пользователя)")
    ...
    author = relationship("User", back_populates="plugins") # Relationship to User
    ...
    def __repr__(self):
        return (
            f"<Plugin(id={self.id}, name='{self.name}', author_id={self.author_id}, version={self.version}, "
